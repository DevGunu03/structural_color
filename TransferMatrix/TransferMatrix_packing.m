...
function R = TransferMatrix_packing(layers, thicknesses, lambda_range, vol_incl_1)
%------------BEGIN USER INPUT PARAMETERS SPECITIFCATION---------------
%
lambda = lambda_range; % Wavelengths over which field patterns are calculated
...
%layers = {'Air' 'aMoO3' 'SiO2' 'aSi'};    % Names of layers of materials starting from side light is incident from
%thicknesses = [0 0 100 50000];       % thickness of each correspondings layer in nm (thickness of the first layer is irrelevant)
%vol_incl = 0.6046;                  % volume portion of inclusion material, add as many as there are number of layers
...
plotGeneration = false;
activeLayer = 2; % index of material layer where photocurrent is generated
%
...
% Additional change introduced for Effective Medium Approximation (EMA)
ema = sum(strcmp(layers, 'PS-beads'));
% Proceed only if 'PS' is present
if ema > 0
    for var = 2:(ema + 1)
        vol_inc = "vol_incl_" + (var - 1);
        n(var, :) = sqrt(MaxwellGarnettFormula(n(1,:).*n(1,:), n(var,:).*n(var,:), eval(vol_inc)));
    end
end
...
% Reflection_legacy=R_glass+T_glass.^2.*R./(1-R_glass.*R);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% MODIFICATION TO CALCULATE SOURCE DEPENDENT REFLECTION
illum_data=readtable('CIE_std_illum_D65.csv');
illum_interp = interp1(illum_data{:,1}, illum_data{:,2}, lambda, 'linear', 'extrap');
Reflection=R_glass+T_glass.^2.*R./(1-R_glass.*R);
reflected = illum_interp .* Reflection;
reflected_weighted = reflected/(12*max(reflected));

 %%
Absorption = 1 - reflected_weighted;  % If transmission is negligible
...
if plotGeneration == true
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% CAN UPDATE SOURCE DATA, FOR AM15, USE 1,2; FOR SLS302, USE 3,5
    % Load in 1sun AM 1.5 solar spectrum in mW/cm2nm
    AM15_data=readtable('AM15.xls');
