...
function R = TransferMatrix_multiple(layers, thicknesses, lambda_range, vol_incl)
...
if ema > 0
    for var = 2:(ema + 1)
        %vol_inc = "vol_incl_l" + (var - 1);
        n(var, :) = sqrt(MaxwellGarnettFormula(n(1,:).*n(1,:), n(var,:).*n(var,:), vol_incl));
    end
end
